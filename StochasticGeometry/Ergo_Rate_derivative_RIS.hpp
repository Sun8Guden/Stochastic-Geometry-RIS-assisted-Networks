...
class Ergo_Rate_RIS_derivative {
...
public:
    Ergo_Rate_RIS_derivative( const Param& param_ ) : imag_unit(0.0, 1.0), param( param_ ) {
    ...
    ~Ergo_Rate_RIS_derivative(){}

    double calc_no_RIS(LT_upsilon_derivative_RIS& lt_trans, const double& s_BF){
    ...
    double calc(LT_upsilon_derivative_RIS& lt_trans, const double& s_BF) {
        ...
        esti_integral_1 = Integration<3>::integrate(f_cpv, cur_region3d, esti_error_1, num_f_eval_1, parallel, 1e-6, 500, 1000000);


        auto f_part2 = [&](const std::array<double, 2>& input2d){
            std::complex<double> lt_vec_part_one = lt_trans.lt_eval(s_BF, input2d.at(0), input2d.at(1) );
            double part_2_val = 0.5 *  lt_vec_part_one.real();
